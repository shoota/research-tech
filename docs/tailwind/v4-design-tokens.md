...
v4 では `@theme` ディレクティブ内で CSS カスタムプロパティとしてデザイントークンを定義する[^1]。名前空間（プレフィックス）ごとに対応するユーティリティクラスが自動生成される。
...
ビルド時にこれらは通常の CSS カスタムプロパティとして `:root` に出力される[^1]:
...
`inline` を使うと、生成されるユーティリティが参照先の変数をそのまま使うため、ダークモード等でスコープが変わっても正しく解決される[^1]。

#### 名前空間のリセット

`initial` キーワードで特定の名前空間やテーマ全体をリセットできる[^1]:
...
v4 では `content` 配列の設定が不要になった[^2]。ヒューリスティクスにより自動的にスキャン対象を決定する:
...
スキャン処理は `@tailwindcss/oxide` パッケージの Rust/NAPI-RS 実装で高速化されている[^5]:
...
**スキャンの特徴**: ファイルをプログラミング言語として解析するのではなく、テキストとしてトークンを検索する[^2]。そのため文字列連結や補間による動的クラス名は検出できない。

#### @source ディレクティブ

自動検出の制御に使う CSS ディレクティブ[^6]:
...
Tailwind v4 のクラス名解決は以下の 3 段階で行われる[^5]:
...
`DesignSystem` クラスがオーケストレーションし、各ユーティリティの `CompileFn` が候補を受け取って AST ノードを生成する[^5]。
...
- **高速スキャン**: Rust 実装の Oxide スキャナーによりフルビルド 3.78x、インクリメンタルビルド最大 182x の高速化を実現[^4]
