...
v4 では `@theme` ディレクティブ内で CSS カスタムプロパティとしてデザイントークンを定義する。名前空間（プレフィックス）ごとに対応するユーティリティクラスが自動生成される。
...
ビルド時にこれらは通常の CSS カスタムプロパティとして `:root` に出力される:
...
`inline` を使うと、生成されるユーティリティが参照先の変数をそのまま使うため、ダークモード等でスコープが変わっても正しく解決される。

#### 名前空間のリセット

`initial` キーワードで特定の名前空間やテーマ全体をリセットできる:
...
v4 では `content` 配列の設定が不要になった。ヒューリスティクスにより自動的にスキャン対象を決定する:
...
スキャン処理は `@tailwindcss/oxide` パッケージの Rust/NAPI-RS 実装で高速化されている:
...
**スキャンの特徴**: ファイルをプログラミング言語として解析するのではなく、テキストとしてトークンを検索する。そのため文字列連結や補間による動的クラス名は検出できない。

#### @source ディレクティブ

自動検出の制御に使う CSS ディレクティブ:
...
Tailwind v4 のクラス名解決は以下の 3 段階で行われる:
...
`DesignSystem` クラスがオーケストレーションし、各ユーティリティの `CompileFn` が候補を受け取って AST ノードを生成する。
...
- **高速スキャン**: Rust 実装の Oxide スキャナーによりフルビルド 3.78x、インクリメンタルビルド最大 182x の高速化を実現
...
- [Detecting classes in source files - Tailwind CSS](https://tailwindcss.com/docs/detecting-classes-in-source-files)
...
- [tailwindlabs/tailwindcss アーキテクチャ解析 - DeepWiki](https://deepwiki.com/tailwindlabs/tailwindcss)
- [Tailwind @source Directive ガイド](https://tailkits.com/blog/tailwind-at-source-directive/)
