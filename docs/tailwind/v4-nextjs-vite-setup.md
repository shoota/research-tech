...
Tailwind CSS v4 は 3 つのビルド統合方式を提供している[^1]。
...
v3 との大きな違いとして、`tailwindcss` パッケージ自体を直接 PostCSS プラグインとして使うことはできなくなった[^5]。代わりに `@tailwindcss/postcss` を使用する必要がある。
...
v4 では `tailwind.config.js` は自動検出されなくなった[^5]。移行には 2 つの方法がある。
...
ただし、v4 では `corePlugins`、`safelist`、`separator` オプションはサポートされない[^5]。
...
セットアップ手順自体に大きな違いはない[^4]。どちらも `@tailwindcss/postcss` を使用し、`globals.css` に `@import "tailwindcss"` を記述する。違いは CSS の読み込み場所のみ。
...
Next.js で CSS Modules と Tailwind を併用する場合、`@reference` ディレクティブを使う[^8]。
...
Next.js 13.1 以降、Turbopack は PostCSS および Tailwind CSS をサポートしている[^9]。`@tailwindcss/postcss` を使用する場合、Turbopack でも特別な設定は不要。
...
- **新しい CSS が不要な変更**（既存クラスの追加・削除）: **192 マイクロ秒**（v3 比 182 倍高速）[^1]
- **新しい CSS が必要な変更**（初めて使うユーティリティの追加）: **5ms**（v3 比 8.8 倍高速）[^1]
...
上記のベンチマーク数値は Tailwind CSS v4.0 リリースブログからの引用[^1]。

### 4. 両環境共通のトピック

#### @theme によるデザイントークンのカスタマイズ

`@theme` ディレクティブは CSS 変数を定義するだけでなく、対応するユーティリティクラスも自動生成する[^6]。
...
v4 ではコンテンツの自動検出が行われ、`.gitignore` に記載されたファイルや `node_modules` は除外される[^7]。
...
// tailwind-merge v3.x は Tailwind CSS v4 をサポート[^10]
...
- パフォーマンスは v3 比でフルビルドが 3.78 倍、インクリメンタルビルドが最大 182 倍高速化しており、開発体験が大幅に向上している[^1]。
- `tailwind-merge` v3.x、CVA、clsx 等のエコシステムライブラリは v4 に対応しており、既存のコンポーネント設計パターンをそのまま利用可能。
- v3 からの移行は `npx @tailwindcss/upgrade` で多くの変更を自動化できるが、ユーティリティ名の変更や `!important` 修飾子の位置変更など、手動確認が必要な箇所もある[^5]。
- `@reference` ディレクティブにより CSS Modules との併用が容易になり、段階的な移行も現実的[^8]。
