...
Tailwind CSS v4 は 3 つのビルド統合方式を提供している。
...
v3 との大きな違いとして、`tailwindcss` パッケージ自体を直接 PostCSS プラグインとして使うことはできなくなった。代わりに `@tailwindcss/postcss` を使用する必要がある。
...
v4 では `tailwind.config.js` は自動検出されなくなった。移行には 2 つの方法がある。
...
ただし、v4 では `corePlugins`、`safelist`、`separator` オプションはサポートされない。
...
セットアップ手順自体に大きな違いはない。どちらも `@tailwindcss/postcss` を使用し、`globals.css` に `@import "tailwindcss"` を記述する。違いは CSS の読み込み場所のみ。
...
Next.js で CSS Modules と Tailwind を併用する場合、`@reference` ディレクティブを使う。
...
Next.js 13.1 以降、Turbopack は PostCSS および Tailwind CSS をサポートしている。`@tailwindcss/postcss` を使用する場合、Turbopack でも特別な設定は不要。
...
- **新しい CSS が不要な変更**（既存クラスの追加・削除）: **192 マイクロ秒**（v3 比 182 倍高速）
- **新しい CSS が必要な変更**（初めて使うユーティリティの追加）: **5ms**（v3 比 8.8 倍高速）
...
`@theme` ディレクティブは CSS 変数を定義するだけでなく、対応するユーティリティクラスも自動生成する。
...
v4 ではコンテンツの自動検出が行われ、`.gitignore` に記載されたファイルや `node_modules` は除外される。
...
// tailwind-merge v3.x は Tailwind CSS v4 をサポート
...
- パフォーマンスは v3 比でフルビルドが 3.78 倍、インクリメンタルビルドが最大 182 倍高速化しており、開発体験が大幅に向上している。
- `tailwind-merge` v3.x、CVA、clsx 等のエコシステムライブラリは v4 に対応しており、既存のコンポーネント設計パターンをそのまま利用可能。
- v3 からの移行は `npx @tailwindcss/upgrade` で多くの変更を自動化できるが、ユーティリティ名の変更や `!important` 修飾子の位置変更など、手動確認が必要な箇所もある。
- `@reference` ディレクティブにより CSS Modules との併用が容易になり、段階的な移行も現実的。

## 参考リンク

- [Tailwind CSS v4.0 リリースブログ](https://tailwindcss.com/blog/tailwindcss-v4)
- [Tailwind CSS v4 インストール - Vite](https://tailwindcss.com/docs/installation/using-vite)
- [Tailwind CSS v4 インストール - PostCSS](https://tailwindcss.com/docs/installation/using-postcss)
- [Tailwind CSS v4 インストール - Next.js](https://tailwindcss.com/docs/installation/framework-guides/nextjs)
- [Tailwind CSS v4 アップグレードガイド](https://tailwindcss.com/docs/upgrade-guide)
- [Tailwind CSS v4 @theme ディレクティブ](https://tailwindcss.com/docs/theme)
- [Tailwind CSS v4 ソースファイルの検出](https://tailwindcss.com/docs/detecting-classes-in-source-files)
- [Tailwind CSS v4 関数とディレクティブ](https://tailwindcss.com/docs/functions-and-directives)
- [Next.js Tailwind CSS ガイド](https://nextjs.org/docs/app/getting-started/css)
