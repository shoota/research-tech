...
Playwright は Microsoft が開発するE2Eテストフレームワークであり、Chromium・Firefox・WebKit のクロスブラウザテストをサポートする[^1]。本ドキュメントでは、Playwright のセットアップからテスト記述、実行方法、レポート出力、CI/CD 統合までを体系的にまとめる。
...
`npm init playwright@latest` を実行すると、対話形式で以下を設定できる[^1]。
...
Playwright はユーザー視点のロケーターを推奨している[^2]。優先度の高い順に以下を使い分ける。
...
Playwright の Web-first アサーションは条件が満たされるまで自動的にリトライする[^12]。
...
ページ固有のロケーターと操作をクラスにまとめ、テストの可読性と保守性を高める[^13]。
...
カスタムフィクスチャを使うと、Page Object の初期化を自動化し、テストコードをさらに簡潔にできる[^3]。
...
セットアッププロジェクトで一度ログインし、認証状態を再利用することでテストを高速化する[^4]。
...
ブラウザを起動せずに API エンドポイントを直接テストできる[^5]。
...
ページやコンポーネントの見た目をピクセル単位で比較し、意図しないUI変更を検出する[^6]。内部では Pixelmatch ライブラリが使用される[^6]。
...
Playwright の ARIA スナップショット機能を使い、アクセシビリティツリーの構造を検証できる[^7]。
...
`playwright.config.ts` で `fullyParallel: true` を設定すると、テストファイル内の各テストケースも並列実行される[^11]。デフォルトではファイル単位で並列化される。
...
テスト結果をインタラクティブなHTML形式で出力する[^8]。ブラウザ・テスト名・ステータスでフィルタリング可能。
...
テスト実行のタイムラインを記録し、各ステップのスクリーンショット・DOM・ネットワークリクエスト・コンソールログを確認できる「タイムトラベルデバッグ」ツール[^9]。
...
CI ツールとの連携やカスタム分析用にJUnit XML・JSON形式で出力できる[^8]。
...
GitHub Actions 上では `github` レポーターを使うと、テスト失敗がアノテーションとして PR に表示される[^10]。
...
導入にあたっては、まずクリティカルなユーザージャーニー（ログイン、主要機能の操作フロー）からテストを書き始め、Page Object Model でテストコードを整理しながら段階的にカバレッジを拡大していく戦略が推奨される[^12]。
