...
shadcn/ui の公式サイトには次のように明記されている[^1]。

> "This is not a component library. It is how you build your component library."

従来のコンポーネントライブラリ（MUI, Ant Design 等）は npm パッケージとしてインストールし、提供された API の範囲内でカスタマイズする。一方、shadcn/ui は CLI を通じてコンポーネントのソースコードをプロジェクトに直接コピーする[^2]。`package.json` に依存関係として追加されるのは Radix UI や Tailwind CSS などの基盤ライブラリのみで、shadcn/ui 自体はランタイム依存に含まれない。

#### コードオーナーシップの思想

shadcn/ui の5つの設計原則は以下の通り[^9]。
...
2025年12月のアップデート以前は、以下の2つのスタイルプリセットが存在した[^6]。
...
2025年12月以降、この2プリセットは5つの新しいビジュアルスタイル（後述）に置き換えられている[^7]。

### 2. Radix UI ベースと Base UI ベースの2系統

#### 経緯

shadcn/ui は当初 Radix UI のみをヘッドレスコンポーネント基盤として採用していた。2025年12月、Base UI（MUI チームが開発するヘッドレス UI ライブラリ）が v1.0 安定版に到達したのを受け、shadcn/ui は `npx shadcn create` コマンドで Base UI を第2の選択肢として正式サポートした[^7]。2026年1月には Base UI 向けの完全なドキュメントが公開された[^8]。
...
- **`render` プロップパターン**: Radix の `asChild` に対応する合成手法として `render` プロップを採用[^10]
- **追加コンポーネント**: マルチセレクト、コンボボックス、オートコンプリート、ナンバーフィールド、メーターなど、Radix にないコンポーネントを提供[^10]
...
`npx shadcn create` で選択できるビジュアルスタイルは以下の5つ[^7][^11]。Radix UI / Base UI いずれの基盤でも利用可能。
...
shadcn/ui は「同じ抽象化、異なるプリミティブ（Same Abstraction, Different Primitives）」を掲げており[^7]、CLI がプロジェクト設定に基づいて正しいバリアントを自動選択する。公式には Radix と Base UI は別々の基盤として機能する設計であり、1つのプロジェクト内での混在は推奨されない[^12]。プロジェクト初期化時にどちらか一方を選択する運用が想定されている。

### 3. テーマシステムの詳細

#### CSS 変数の命名規約

shadcn/ui のテーマは CSS カスタムプロパティ（CSS 変数）で構成される[^3]。命名は `background` / `foreground` ペア規約に従う。
...
shadcn/ui は OKLCH（Oklab Lightness Chroma Hue）カラースペースを採用している[^3]。OKLCH は知覚的に均一な色空間であり、以下の利点がある。
...
Tailwind v4 では `@theme inline` ディレクティブを使用して CSS 変数を Tailwind のユーティリティクラスとして公開する[^4]。
...
ダークモードは `.dark` クラスベースの戦略で実装される[^5]。
...
Next.js では `next-themes` ライブラリとの組み合わせが推奨されている[^5]。
...
shadcn/ui の Tailwind v3 → v4 対応で発生する主な変更点[^4]:
...
`data-slot` 属性により、親コンポーネントから子のスタイリングを Tailwind で制御しやすくなった[^4]:
...
`npx shadcn create` を実行すると、対話的に以下の選択を行う[^7]。
