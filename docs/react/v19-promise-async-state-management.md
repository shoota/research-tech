...
React v19で導入された `use` フック、Actions、`useActionState`、`useOptimistic`、`useTransition` の非同期拡張、およびServer Componentsの正式サポートにより、非同期データ取得と状態管理のパラダイムがどのように変化したかを調査した。
...
React v18では `Suspense` が限定的にサポートされていたが、主に `React.lazy()` によるコード分割用途に留まっていた。データ取得でのSuspense利用は「実験的」とされ、Relay等の特定フレームワーク経由でのみ推奨されていた。
...
`use` はReact v19で導入された新しいAPIで、Promise やコンテキストをレンダリング中に読み取ることができる。最大の特徴は、**従来のフックルールに縛られず、条件分岐やループ内でも呼び出せる**点にある。
...
**重要な制約**: `use` に渡すPromiseはレンダリング中に生成してはならない。Server ComponentからClient Componentに渡されたPromise、またはSuspense対応ライブラリから取得したPromiseを使用する必要がある。

#### Actions: 非同期処理を宣言的に扱う仕組み

React v19では「Action」という概念が導入された。Actionとは、`useTransition` 内の非同期関数、`<form>` の `action` 属性に渡す関数などを指す。Actionsは以下を自動的に提供する:
...
`useActionState` はActionの結果（状態、Pending状態、エラー）を一括管理するフック。従来の `isLoading`, `hasError`, `isSuccess` といった複数の `useState` を1つに集約する。
...
`useOptimistic` は、非同期処理の完了を待たずにUIを即座に更新し、処理が完了またはエラーになった時点で実際の値に切り替えるフック。
...
React v18の `useTransition` は同期的なコールバックしか受け付けなかったが、v19では非同期関数を渡せるようになった。これにより `isPending` の手動管理が不要になる。
...
`isPending` は非同期処理の開始時に自動的に `true` になり、完了時に `false` にリセットされる。`setIsPending(true)` / `setIsPending(false)` の手動呼び出しが不要になった。
...
React v19ではSuspenseが「遅延ロードの補助」から「非同期レンダリングの中核的な調整メカニズム」へと昇格した。Suspenseはローディング機能ではなく、レンダリング調整メカニズムである。コンポーネントが「まだレンダリングの準備ができていない」とReactに伝えると、Reactはそのツリー部分のレンダリングを一時停止し、データが利用可能になるまでfallback UIを表示する。
...
React v19ではエラーハンドリングも改善された。v18以前ではエラーが二重にスローされていたが（元のエラー + 自動回復失敗時のエラー）、v19では重複が排除された。
...
`Suspense` 内で `use()` に渡したPromiseがrejectされると、そのエラーは最も近い `ErrorBoundary` に伝播する。これにより、各コンポーネント内での `try-catch` や `error` state が不要になる。
...
Server Componentsはサーバー上で実行され、async/awaitを直接使用できる。Client Componentsではasync/awaitは使えない（`use` フックを代わりに使う）。
...
`"use server"` ディレクティブで定義された非同期関数は、Client Componentから直接呼び出すことができる。フレームワークがサーバー関数への参照を自動生成し、クライアントで呼び出されるとReactがサーバーにリクエストを送信して関数を実行する。
...
Server ComponentからClient ComponentへPromiseを渡すパターンは、React v19の重要な設計パターンである。
...
React v19の `use` フックでは、Promiseの参照自体がpropsとして渡されるため、propsが変わればReactが自動的に新しいPromiseを使用する。古いPromiseの結果は無視され、レースコンディションが構造的に発生しない。
...
Render-as-you-Fetchパターンでは、データ取得がコンポーネントのレンダリングから切り離されるため、レンダリングの階層構造に起因するウォーターフォールが構造的に発生しない。
...
| `useActionState` | 安定版 | 旧 `useFormState` から改名 |
| `useOptimistic` | 安定版 | |
...
- `use` に渡すPromiseは**レンダリング中に生成してはいけない**。親コンポーネントやServer Component、キャッシュ層で事前に生成する必要がある
- Client Componentは `async` 関数として定義できない。非同期処理は `use` フック経由で行う
- Server ComponentsとServer Actionsは、Next.js等のフレームワークとの統合が前提
- `useActionState` は `react` パッケージから、`useFormStatus` は `react-dom` パッケージからインポートする
...
- [React v19 公式リリースブログ](https://react.dev/blog/2024/12/05/react-19)
- [use - React 公式リファレンス](https://react.dev/reference/react/use)
- [useActionState - React 公式リファレンス](https://react.dev/reference/react/useActionState)
- [useOptimistic - React 公式リファレンス](https://react.dev/reference/react/useOptimistic)
- [useTransition - React 公式リファレンス](https://react.dev/reference/react/useTransition)
- [Suspense - React 公式リファレンス](https://react.dev/reference/react/Suspense)
- [Server Components - React 公式リファレンス](https://react.dev/reference/rsc/server-components)
- [Server Functions - React 公式リファレンス](https://react.dev/reference/rsc/server-functions)
- ['use server' directive - React 公式リファレンス](https://react.dev/reference/rsc/use-server)
- [The Complete Developer Guide to React 19: Async Handling - Callstack](https://www.callstack.com/blog/the-complete-developer-guide-to-react-19-part-1-async-handling)
...
- [Smooth Async Transitions in React 19 - AppSignal Blog](https://blog.appsignal.com/2025/08/27/smooth-async-transitions-in-react-19.html)
...
- [Mastering React 19: Server Components & Server Actions - Scalable Path](https://www.scalablepath.com/react/react-19-server-components-server-actions)
- [Getting started with startTransition in React 19 - LogRocket](https://blog.logrocket.com/getting-started-react-19-starttransition/)
